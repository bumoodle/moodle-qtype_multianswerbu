...
class qtype_multianswerbu_edit_form extends qtype_scripted_edit_form 
{
    ...
    //  $savedquestiondisplay will contain the qtype_multianswerbu_extract_question
    ...
    protected function definition_inner($mform) {
        ...
        {
            $this->questiondisplay = fullclone(qtype_multianswerbu_extract_question($questiontext));
        } else {
        ...
        //add the "analyze" question button
        ...
        if ($this->reload) {
            for ($sub = 1; $sub <= $countsubquestions; $sub++) {
                ...
                if (isset($this->savedquestiondisplay->options->questions[$sub]->qtype) &&
                    ...
                    $storemess = ' ' . html_writer::tag('span', get_string(
                            'storedqtype', 'qtype_multianswerbu', question_bank::get_qtype_name(
                                    $this->savedquestiondisplay->options->questions[$sub]->qtype)),
                            array('class' => 'error'));
                }
                ...
                $mform->addElement('static', 'sub_'.$sub.'_questiontext',
                        get_string('questiondefinition', 'qtype_multianswerbu'));
                ...
                if ($this->questiondisplay->options->questions[$sub]->qtype == 'multichoice') {
                    $mform->addElement('static', 'sub_'.$sub.'_layout',
                            get_string('layout', 'qtype_multianswerbu'));
                }
            ...
            if (($this->negative_diff > 0) ||$this->qtype_change ||
                ...
                $mform->addElement('header', 'additemhdr',
                        get_string('warningquestionmodified', 'qtype_multianswerbu'));
            }
            if ($this->negative_diff > 0) {
                $mform->addElement('static', 'alert1', "<strong>".
                        get_string('questiondeleted', 'qtype_multianswerbu')."</strong>",
                        get_string('questionsless', 'qtype_multianswerbu', $this->negative_diff));
            }
            if ($this->qtype_change) {
                $mform->addElement('static', 'alert1', "<strong>".
                        get_string('questiontypechanged', 'qtype_multianswerbu')."</strong>",
                        get_string('questiontypechangedcomment', 'qtype_multianswerbu'));
            }
        }
        if ($this->used_in_quiz) {
            if ($this->negative_diff < 0) {
                ...
                $mform->addElement('static', 'alert1', "<strong>".
                        get_string('questionsadded', 'qtype_multianswerbu')."</strong>",
                        "<strong>".get_string('questionsmore', 'qtype_multianswerbu', $diff).
                        "</strong>");
            }
            $a = new stdClass();
            ...
            $mform->addElement('header', 'additemhdr2',
                    get_string('questionusedinquiz', 'qtype_multianswerbu', $a));
            $mform->addElement('static', 'alertas',
                    get_string('youshouldnot', 'qtype_multianswerbu'));
        }
        if (($this->negative_diff > 0 || $this->used_in_quiz &&
                ($this->negative_diff > 0 || $this->negative_diff < 0 || $this->qtype_change)) &&
                        $this->reload) {
            $mform->addElement('header', 'additemhdr',
                    get_string('questionsaveasedited', 'qtype_multianswerbu'));
            $mform->addElement('checkbox', 'confirm', '',
                    get_string('confirmquestionsaveasedited', 'qtype_multianswerbu'));
    ...
    public function set_data($question) {
        ...
        if (isset($question->id) and $question->id and $question->qtype &&
            ...
            foreach ($question->options->questions as $key => $wrapped) {
                if (!empty($wrapped)) {
                    ...
                    // update all multianswerbu questions
                            ...
                            switch ($subquestion->layout) {
                                case '0':
                                    $default_values[$prefix.'layout'] =
                                            get_string('layoutselectinline', 'qtype_multianswerbu');
                                    break;
                                case '1':
                                    $default_values[$prefix.'layout'] =
                                            get_string('layoutvertical', 'qtype_multianswerbu');
                                    break;
                                case '2':
                                    $default_values[$prefix.'layout'] =
                                            get_string('layouthorizontal', 'qtype_multianswerbu');
                                    break;
                                default:
                                    $default_values[$prefix.'layout'] =
                                            get_string('layoutundefined', 'qtype_multianswerbu');
                            }
                        }
        ...
        $default_values['alertas']= "<strong>".get_string('questioninquiz', 'qtype_multianswerbu').
                "</strong>";
    ...
    public function validation($data, $files) {
        ...
        if (isset($questiondisplay->options->questions)) {
            ...
            if (count($subquestions)) {
                ...
                $errors['questiontext'] = get_string('questionsmissing', 'qtype_multianswerbu');
            }
        }
        ...
        if (($this->negative_diff > 0 || $this->used_in_quiz &&
            ...
            $errors['confirm'] =
                    get_string('confirmsave', 'qtype_multianswerbu', $this->negative_diff);
        }
