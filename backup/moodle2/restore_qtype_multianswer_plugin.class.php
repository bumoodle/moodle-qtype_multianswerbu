 ...
 * needed to restore one multianswerbu qtype plugin
 *
 * @copyright  2010 onwards Eloy Lafuente (stronk7) {@link http://stronk7.com}
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class restore_qtype_multianswerbu_plugin extends restore_qtype_plugin {
    ...
    protected function define_question_plugin_structure() {
        ...
        // Add own qtype stuff
        $elename = 'multianswerbu';
        $elepath = $this->get_pathfor('/multianswerbu');
    ...
    /**
     * Process the qtype/multianswerbu element
     */
    public function process_multianswerbu($data) {
        global $DB;
        ...
        // question_multianswerbu too
        if ($questioncreated) {
            // Adjust some columns
            $data->question = $newquestionid;
            // Note: multianswerbu->sequence is a list of question->id values. We aren't
            ...
            // Insert record
            $newitemid = $DB->insert_record('question_multianswerbu', $data);
            // Create mapping (need it for after_execute recode of sequence)
            $this->set_mapping('question_multianswerbu', $oldid, $newitemid);
        }
    }
     ...
     *
     * For multianswerbu qtype we use it to restore the sequence column,
    ...
    public function after_execute_question() {
        ...
        // the created question_multianswerbu sequences (list of question ids)
        $rs = $DB->get_recordset_sql("
                SELECT qma.id, qma.sequence
                  FROM {question_multianswerbu} qma
        ...
        foreach ($rs as $rec) {
            ...
            $DB->set_field('question_multianswerbu', 'sequence', $sequence,
                    array('id' => $rec->id));
        }
        $rs->close();
    }
    ...
    public function recode_response($questionid, $sequencenumber, array $response) {
        ...
        $sequence = $DB->get_field('question_multianswerbu', 'sequence',
     ...
     * recoded pointing to all the restored stuff for multianswerbu questions
     *
     * answer is one comma separated list of hypen separated pairs
     * containing sequence (pointing to questions sequence in question_multianswerbu)
